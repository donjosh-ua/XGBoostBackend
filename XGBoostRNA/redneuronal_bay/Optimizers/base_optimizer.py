...
class BaseOptimizer():
    ...
    def _forwardprop(self,Rn,x,run,trace=True):
        ...
        for i in range(len(Rn.layersObject)):
            ...
            Output = layer.funcion_activacion(torch.add(torch.matmul(enput, layer.weights), layer.bias),a)            
    ...
    def backprop(self,xy,Rn,save_mod,verbose,cv,k):
        ...
        class RunningMetric(): # Esta funcion me permite que se vaya obteniendo valores promedio del traajo de la red,
            ...
            def __call__(self):
                return self.S/float(self.N) # float para tenre un resultado flotante y no una division entera
        ...
        while run < Rn.epoch:
            ...
            for inputs, targets in xy:
                ...
                for i in reversedlayers:
                    ...
                    if i == outputlayerindex:
                        ...
                        loss_b = np.append(loss_b,(Losgr.data/num_batches)) #Para graficar funcion de perdida
                        
                        #iteraciones += 1
                                                
                        running_loss.update(Losgr*n_total_row,n_total_row ) 
                        running_acc.update(ac1.float(),n_total_row ) # Actualizacion y casteo a flotante 
                        ...
                        delta = self.calculate_delta(Rn.layersObject[i].derivative(each_layer_output[i+1][0])
                    ...
                    Rn.layersObject[i].weights = self.update_weights(delta, layer_input=each_layer_output[i][0],
                                                layer_index=i,curr_weights=Rn.layersObject[i].weights, Lambda=Rn.Lambda,
                                                                     Bay=self.Bay,img=self.img,image_size=self.image_size,ite_act=ite_act,
                                                                        total=num_batches, acbay =actbay)
                    ##print(f'Ajustado peso: {i}')
                    ##print(Rn.layersObject[i].weights)
                    ##print(Rn.layersObject[i].weights.shape)

                    Rn.layersObject[i].bias = self.update_bias(delta, curr_bias=Rn.layersObject[i].bias,Bay=self.Bay,img=self.img,                                                                                ite_act=ite_act,total=num_batches, layer_index=i, acbay =actbay)
                ...
                ite_act += 1
                if Rn.verbose == True:                    
                    progreso_epo(ite_act=ite_act, total=num_batches,total_epoch=Rn.epoch, act_epoch=run+1, prefix='Progress:')
            ...
            if Rn.verbose == True:
                ...
                print(f'learning rate:{self.learning_rate} loss:{np.round(ru_los*1.0,4)}')
        ...
        torch.save(ac_b2, n1)
        torch.save(loss_b2, n2)
        torch.save(run,'epoch') # Se podria quitar ojo revisar bien 
        #-------------------------------------------------------------------------------
        ...
        print('---------------------------------------')
        torch.save(Rn.layersObject, name_model)
        ...
        salid = Rn.layersObject # Se realiza esto para que guarde el modelo que tendra como salida y aplicar la funcion para
        #                          que los pesos regresen a un valor aleatorio (esto es para CV) - en utils act_Weig_bias            
        act_Weig_bias(Rn)
    ...
    @abstractmethod
    def update_weights(self,delta, layer_input, layer_index,curr_weights, Lambda, Bay, rseed):
        '''
    ...
    def update_bias(self, delta, curr_bias, Bay, img, ite_act, total, layer_index, acbay):
        '''
        Al ser una funcion abstracta se divide para cada clase derivada
        :delta:
        ...
        if Bay==True and ite_act==(total-1) and acbay==1: # Para que haga el bayesiano en el ultimo batch de cada epoch
            ...
            with pm.Model() as NNBb:
                ...
                delta_b = pm.Deterministic('delta_b',(tt.sum(delt_b, axis=0, keepdims=True)))
                #--delta_b = tt.sum(delta, axis=0, keepdims=True)

                # Defino likelihood en funcion de los valores establecidos y priors
                obs_pos_b = pm.Normal('obs_pos_b', mu=delta_b, sigma=sd_b,observed=delta_b)
                ...
                trace = pm.sample(5, step=step, tune=tunb, cores=4, chains=1, compute_convergence_checks=False, 
                                  progressbar=True)    
